...
$order_id = "IV-23-AF|IV-34-AF";// multiple
...
$v_code = gen_hash($merchant_id,$order_id,$v_key); // transaction id
...
<input type=hidden name=oIDs value='<?php echo $order_id ?>'>
<input type=hidden name=domain value='<?php echo $merchant_id ?>'>
<input type=hidden name=skey value='<?php echo $v_code ?>'>
<input type=hidden name=url value='http://localhost/razorpay/callback_rms_oid.php'>
...
function gen_hash($merchant_id,$oid,$vkey){
    $merchantID = $merchant_id;
    $orderid = $oid;
    $verifykey = $vkey;
    $vcode = md5($merchantID.$orderid.$verifykey );
